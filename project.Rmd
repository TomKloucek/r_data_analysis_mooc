...
ggplot(data, aes(x = reorder(club, headers), y = headers)) +
  ...
  labs(title = "Teams' Header Goals in Liga Mistrů",
       x = "Club",
       y = "Number of Headers Scored") +
  theme_minimal() +
  coord_flip()
...
ggplot(goal_counts, aes(x = goal_type, y = count, fill = goal_type)) +
  ...
  labs(title = "Most Common Types of Goals in Liga Mistrů",
       x = "Goal Type",
       y = "Count") +
  theme_minimal()
```
...
ggplot(goal_counts, aes(x = goal_type, y = count, fill = position)) +
  ...
  labs(title = "Comparison of Goal Types by Position in Liga Mistrů",
       x = "Goal Type",
       y = "Count") +
  theme_minimal() +
...
# Seřadit data podle průměrných bodů sestupně a vybrat prvních 10 hráčů
...
ggplot(data = top_10_players, aes(x = reorder(player_name, -score), y = score)) +
  ...
  labs(title = "Player Scores in the Tournament by match",
       x = "Player Name",
       y = "Score") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
ggplot(data = top_10_players_bytotal, aes(x = reorder(player_name, -total_score), y = total_score)) +
  ...
  labs(title = "Player Scores in the Tournament",
       x = "Player Name",
       y = "Score") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
```
